...
## Brownfield/Import Modules — Exception to Rules 1 & 3

Everything below assumes the module is a **reusable asset** meant to be launched multiple times.
A module generated by `import-cloud-resources-as-environment` (or any Terraform wrapping an
already-existing, already-running resource or set of resources brought under Torque management)
is a different, equally valid category with the opposite defaults — not a lesser or unfinished
version of a normal module. Full reasoning: `import-cloud-resources-as-environment`'s
`references/track-a-vs-track-b.md`.

**Recognize it:** the user says "import," "brownfield," "codify this resource," or the module is
flat literal resource blocks with no `variables.tf` at all — that shape is the correct, intended
first cut for such a module, not an incomplete one.

**What changes for a brownfield/import (Track A) module:**
- **Rule 1 (parameterize uniqueness-constrained attributes) does not apply.** A bucket name, IAM
  role name, or DB identifier that's hardcoded to match a resource that already exists in the
  cloud is correct — the name isn't a design choice waiting to be parameterized, it's a fact about
  a resource that already has that name. Do not flag it, and do not "fix" it by extracting a
  `var.env_id`.
- **Rule 3 (avoid over-parameterization) becomes even stricter.** The default is zero variables.
  Only add an input if the user explicitly wants to be able to *update* the live resource(s)
  through Torque (instance size, tags, replica counts) — never to make the module launchable as a
  second, independent copy. If a field is pinned by `lifecycle.ignore_changes` or is
  creation-time-only, an input wired to it is inert against the resource it targets; say so in the
  input's description rather than presenting it as an effective one.
- **Rules 2, 4–8 still apply as written** — outputs, provider versioning, secrets handling, and
  state hygiene don't change based on whether the module is a template or a live-resource wrapper.

**If the user wants this module to also work as a template (Track B)** — a digital twin, or
templatizing a long-running environment — say explicitly that this is a different, rarer case
(`references/track-a-vs-track-b.md`'s two bars: completeness and full parameterization), and then
apply Rules 1 and 3 in full, exactly as written below. Never make this judgment call silently;
confirm with the user before parameterizing an import-generated module as if it were reusable.

### Traps specific to import-generated configuration

These recur across every import regardless of cloud, precisely because import output is written
as flat literals with no relationships between resources:

- **Not every same-named field means the same thing — can destroy the resource on `apply`.** A
  cluster resource and a node-pool resource can both expose a field with the same name (e.g.
  `initial_node_count`); on one it's an ordinary creation-time count, on the other it can force
  full replacement. Check whether a field forces replacement **on the specific resource type**
  before parameterizing it (Rule 1 exception above notwithstanding) — read the provider docs for
  that resource, not just the field name.
- **Let outside owners own their fields.** When something outside Terraform legitimately changes a
  value (a managed auto-upgrade, an external autoscaler), Terraform proposes reverting it on every
  `plan` forever unless it's named in `lifecycle.ignore_changes`.
- **No relationships means silent drift.** A hardcoded network/subnet ID string where a `data`
  source could instead resolve the real resource means a rename or deletion elsewhere never
  surfaces at `plan` time — it just silently stops matching. Prefer a data source over a bare
  literal for anything the module doesn't own but depends on.
- **Keep the `backend` block out of the module entirely** — for an import, doubly so: the backend
  key is fixed to one already-existing state file, and it belongs in the blueprint's `spec.backend`
  or the Torque import API's `grains[].backend`, never in the committed `.tf` source (see
  `author-blueprint` and `import-cloud-resources-as-environment` for where it actually goes).

---

## The 8 Rules

**The rules below assume a reusable, multi-launch module.** For a brownfield/import module, see
the exception above before applying Rules 1 and 3.
