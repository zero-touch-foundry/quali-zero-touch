...
1. **Server-side validation** — run the authoritative schema check via the Torque REST API:
   ...
   This checks spec_version, grain kinds, schema, and Liquid reference resolution. Parse the resulting JSON `{is_valid, errors, warnings}` (shape in `skills/torque-api/references/response_shapes.md`). Report every `errors[]` entry verbatim with `line` / `column` / `path` when present.
...
If `TORQUE_API_TOKEN` is unset or the call fails with a network error (`HTTP 0`), fall back to the `torque-blueprint-reviewer` skill alone and note that server-side validation was skipped. For 401/403/404 surface the fix from `skills/torque-api/references/errors.md` and stop.
